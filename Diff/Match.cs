// Imported from: https://github.com/dotnet/roslyn/blob/version-2.10.0/src/Workspaces/Core/Portable/Differencing/Match.cs
    ...
    public sealed partial class Match<TNode>
    {
        ...
        internal Match(TNode root1, TNode root2, TreeComparer<TNode> comparer, IEnumerable<KeyValuePair<TNode, TNode>> knownMatches)
        {
            ...
            if (knownMatches != null)
            {
                foreach (var knownMatch in knownMatches)
                {
                    ...
                    {
                        throw new ArgumentException(string.Format("Matching nodes {0} and {1}must have the same label", knownMatch.Key, knownMatch.Value), nameof(knownMatches));
                    }

                    if (!comparer.TreesEqual(knownMatch.Key, root1))
                    {
                        throw new ArgumentException(string.Format("Node {0} must be contained in  the old tree", knownMatch.Key), nameof(knownMatches));
                    }

                    if (!comparer.TreesEqual(knownMatch.Value, root2))
                    {
                        throw new ArgumentException(string.Format("Node {0} must be contained in the new tree", knownMatch.Value), nameof(knownMatches));
                    }
            ...
            out int totalCount)
        {
            ...
            foreach (TNode node in comparer.GetDescendants(root))
            {
                ...
                {
                    throw new InvalidOperationException(string.Format("Label for node {0} is invalid, it must be within [0, {1}]", node, labelCount));
                }
