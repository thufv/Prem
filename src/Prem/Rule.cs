...
namespace Prem
{
    ...
    public class RuleSet : ASTSerialization.IObjSerializable
    {
        ...
        public string Name => "from " + synthesizedFrom;
        ...
        public RuleSet(ErrPattern pattern, List<TProgram> programs, string resource)
        {
            this.errPattern = pattern;
            this.transformers = programs;
            this.synthesizedFrom = resource;
        }
        ...
        public Optional<int> TestAll(Example example)
        {
            ...
            {
                Log.Debug("Pattern {0} cannot match against \"{1}\".", errPattern, example.input.errMessage);
        ...
        public XElement serialize()
        {
            ...
            var synthesizedFrom_xe = new XElement("Attr-synthesizedFrom");
            ASTSerialization.Serialization.fillXElement(synthesizedFrom, synthesizedFrom_xe);
            xe.Add(errPattern_xe);
            xe.Add(transformers_xe);
            xe.Add(synthesizedFrom_xe);
            return xe;
        }
        ...
        public RuleSet(XElement xe)
        {
            ...
            synthesizedFrom = ASTSerialization.Serialization.makeObject(xe.Element("Attr-synthesizedFrom")) as string;
        }
    }
    ...
    public class RuleLib : ASTSerialization.IObjSerializable
    {
        ...
        public abstract class ApplyResult<T> {}
        
        public class ApplySuccess<T> : ApplyResult<T>
        {
            public int applyTry { get; }
            
            public string ruleUsed { get; }

            public ApplySuccess(int k, string rule)
            {
                this.applyTry = k;
                this.ruleUsed = rule;
            }
        ...
        public class ApplyFailure<T> : ApplyResult<T>
        {
            public T error { get; }

            public string ruleUsed { get; }

            public ApplyFailure(T error, string rule)
            {
                this.error = error;
                this.ruleUsed = rule;
            }
        ...
        public class Unapplicable<T> : ApplyResult<T> {}

        public ApplyResult<T> Apply<T>(Input testcase, Func<SyntaxNode, Optional<T>> validator)
        {
            int applyTry = 0;
            T lastErr = default(T);
            string lastRule = "";
            ...
            foreach (var ruleSet in ruleSets) // try each rule set
            {
                var env = new Env();
                if (ruleSet.errPattern.Match(testcase.errMessage, env)) // the rule set is appliable
                {
                    var input = testcase.AsTInput(env);
                    foreach (var transformer in ruleSet.transformers) // try each rule in order
                    {
                        applyTry++;
                        var r = transformer.Apply(input);
                        if (r.HasValue) // rule is applicable without exceptions
                        {
                            var err = validator(r.Value);
                            if (err.HasValue)
                            {
                                hasErr = true;
                                lastErr = err.Value;
                                lastRule = ruleSet.Name;
                                // continue attempting
                            }
                            else
                            {
                                return new ApplySuccess<T>(applyTry, ruleSet.Name);
                            }
                        }
                        // rule is not applicable
                    }
                }
                // else: the entire rule set is not applicable
            }

            if (hasErr) return new ApplyFailure<T>(lastErr, lastRule);
            return new Unapplicable<T>();
        }
